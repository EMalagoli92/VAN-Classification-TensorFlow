...
class LKA(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.conv0 = Conv2d_(in_channels = self.dim,
        ...
        self.conv_spatial = Conv2d_(in_channels = self.dim,
                                    out_channels = self.dim,
                                    kernel_size = 7,
                                    stride = 1,
                                    padding = 9,
                                    groups = self.dim,
                                    dilation = 3,
                                    kernel_initializer = CustomNormalInitializer(kernel_size = 7, out_channels = self.dim, groups = self.dim),
                                    bias_initializer = tf.keras.initializers.Zeros(),
                                    name = "conv_spatial"
                                    )
        self.conv1 = Conv2d_(in_channels = self.dim,
                             out_channels = self.dim,
                             kernel_size = 1,
                             kernel_initializer = CustomNormalInitializer(kernel_size = 1, out_channels = self.dim),
                             bias_initializer = tf.keras.initializers.Zeros(),
                             name = "conv1"
                             )
