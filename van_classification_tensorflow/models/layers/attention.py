...
from vab_classification_tensorflow.models.layers.lka import LKA
...
class Attention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.proj_1 = Conv2d_(in_channels = self.d_model,
                              out_channels = self.d_model,
                              kernel_size = 1,
                              kernel_initializer = CustomNormalInitializer(kernel_size = 1, out_channels = self.d_model),
        ...
        self.proj_2 = Conv2d_(in_channels = self.d_model,
                              out_channels = self.d_model,
                              kernel_size = 1,
                              kernel_initializer = CustomNormalInitializer(kernel_size = 1, out_channels = self.d_model),
                              bias_initializer = tf.keras.initializers.Zeros(),
