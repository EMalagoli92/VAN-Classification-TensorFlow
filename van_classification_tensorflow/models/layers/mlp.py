...
class Mlp(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.fc1 = Conv2d_(in_channels = self.in_features,
                           out_channels = self._hidden_features,
                           kernel_size = 1,
                           kernel_initializer = CustomNormalInitializer(kernel_size = 1, out_channels = self._hidden_features),
                           bias_initializer = tf.keras.initializers.Zeros(),
                           name = "fc1"
                           )
        self.dwconv = DWConv(dim = self._hidden_features,
                             name = "dwconv"
                             )
        if self.act_layer == "gelu":
            self.act = tfa.layers.GELU(approximate=False, name="act")
        else:
            self.act = tf.keras.layers.Activation(self.act_layer, dtype=self.dtype, name="act")
        
        self.fc2 = Conv2d_(in_channels = self._hidden_features,
                           out_channels = self._out_features,
                           kernel_size = 1,
                           kernel_initializer = CustomNormalInitializer(kernel_size = 1, out_channels = self._out_features),
                           bias_initializer = tf.keras.initializers.Zeros(),
                           name = "fc2"
                           )
